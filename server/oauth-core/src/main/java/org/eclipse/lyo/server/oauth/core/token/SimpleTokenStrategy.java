...
public class SimpleTokenStrategy implements TokenStrategy {
    ...
    public void generateRequestToken(OAuthRequest oAuthRequest) throws IOException {
        ...
        String callback = oAuthRequest.getMessage().getParameter(OAuth.OAUTH_CALLBACK);
        synchronized (requestTokens) {
            requestTokens.put(
                    accessor.requestToken,
                    new RequestTokenData(accessor.consumer.consumerKey, callback));
        }
    ...
    @Override
    public String validateRequestToken(HttpServletRequest httpRequest, OAuthMessage message)
            throws OAuthException, IOException {
        return getRequestTokenData(message.getToken()).getConsumerKey();
    }

    @Override
    public String getCallback(HttpServletRequest httpRequest, String requestToken)
    ...
    @Override
    public String generateVerificationCode(HttpServletRequest httpRequest, String requestToken)
            throws OAuthProblemException {
    ...
    public void validateVerificationCode(OAuthRequest oAuthRequest)
            throws OAuthException, IOException {
        String verificationCode = oAuthRequest.getMessage().getParameter(OAuth.OAUTH_VERIFIER);
    ...
    public void generateAccessToken(OAuthRequest oAuthRequest)
        ...
        synchronized (requestTokens) {
            if (!isRequestTokenAuthorized(oAuthRequest.getHttpRequest(), requestToken)) {
        ...
        synchronized (accessTokens) {
            accessTokens.put(accessor.accessToken, accessor.consumer.consumerKey);
        }
