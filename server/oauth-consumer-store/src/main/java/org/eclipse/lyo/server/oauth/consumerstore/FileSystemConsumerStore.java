...
public class FileSystemConsumerStore extends AbstractConsumerStore {
    ...
    protected static final String CONSUMER_RESOURCE = LYO_OAUTH_NAMESPACE + "Consumer";
    protected static final String CALLBACK_URL = LYO_OAUTH_NAMESPACE + "callback";
    protected static final String CONSUMER_NAME = LYO_OAUTH_NAMESPACE + "consumerName";
    protected static final String CONSUMER_KEY = LYO_OAUTH_NAMESPACE + "consumerKey";
    protected static final String CONSUMER_SECRET = LYO_OAUTH_NAMESPACE + "consumerSecret";
    protected static final String PROVISIONAL = LYO_OAUTH_NAMESPACE + "provisional";
    protected static final String TRUSTED = LYO_OAUTH_NAMESPACE + "trusted";
    ...
    public FileSystemConsumerStore(String oauthStoreRoot)
            throws SQLException, ConsumerStoreException, ClassNotFoundException {
     ...
     */
    public FileSystemConsumerStore(String oauthStoreRoot, String encryptionKey)
            throws SQLException, ConsumerStoreException, ClassNotFoundException {
        this.oauthStore = oauthStoreRoot;
    ...
    public FileSystemConsumerStore(Model model, String oauthStoreRoot)
            throws ConsumerStoreException {
        this.oauthStore = oauthStoreRoot;
        this.model = model;
        loadConsumers();
    ...
    protected synchronized void loadConsumers() throws ConsumerStoreException {
        ResIterator i =
                model.listResourcesWithProperty(RDF.type, model.createResource(CONSUMER_RESOURCE));
        while (i.hasNext()) {
            ...
            try {
                ...
                log.error(
                        "Could not load consumer "
                                + consumerResource.getProperty(model.createProperty(CONSUMER_NAME))
                                + " ("
                                + consumerResource.getProperty(model.createProperty(CONSUMER_KEY))
                                + ")",
                        e);
    ...
    protected void removeProperties(String consumerKey) {
        ResIterator i =
                model.listResourcesWithProperty(
                        model.createProperty(CONSUMER_KEY), model.createLiteral(consumerKey));
        while (i.hasNext()) {
    ...
    protected Resource toResource(LyoOAuthConsumer consumer) throws UnsupportedEncodingException {
        ...
        resource.addProperty(model.createProperty(CONSUMER_NAME), consumer.getName());
        resource.addProperty(model.createProperty(CONSUMER_KEY), consumer.consumerKey);
        ...
        if (this.encryptionKey != null) { // if key is present use AES 128 encryption
            ...
            encodedSecret =
                    new String(
                            Base64.getEncoder().encode(consumer.consumerSecret.getBytes("UTF8")),
                            "UTF8");
        }
        resource.addProperty(model.createProperty(CONSUMER_SECRET), encodedSecret);

        resource.addProperty(
                model.createProperty(PROVISIONAL), (consumer.isProvisional()) ? "true" : "false");
        resource.addProperty(
                model.createProperty(TRUSTED), (consumer.isTrusted()) ? "true" : "false");

        return resource;
    }

    protected LyoOAuthConsumer fromResource(Resource resource) throws UnsupportedEncodingException {
        String key = resource.getRequiredProperty(model.createProperty(CONSUMER_KEY)).getString();

        String encodedSecret =
                resource.getRequiredProperty(model.createProperty(CONSUMER_SECRET)).getString();
        String secret = null;
        ...
        consumer.setName(
                resource.getRequiredProperty(model.createProperty(CONSUMER_NAME)).getString());

        String provisional = resource.getProperty(model.createProperty(PROVISIONAL)).getString();
        consumer.setProvisional("true".equals(provisional));

        String trusted = resource.getProperty(model.createProperty(TRUSTED)).getString();
