...
public class RdfConsumerStore extends AbstractConsumerStore {
    ...
    protected static final String DB = "RDFStore"; // database type
        ...
        try {
            ...
            ResIterator i =
                    model.listResourcesWithProperty(
                            RDF.type, model.createResource(CONSUMER_RESOURCE));
            while (i.hasNext()) {
                ...
                try {
                    ...
                    log.error(
                            "Could not load consumer "
                                    + consumerResource.getProperty(
                                            model.createProperty(CONSUMER_NAME))
                                    + " ("
                                    + consumerResource.getProperty(
    ...
    @Override
    public LyoOAuthConsumer removeConsumer(final String consumerKey) throws ConsumerStoreException {
        if (model == null) {
            throw new ConsumerStoreException("Consumer store not initialized.");
        }
    ...
    protected Resource toResource(LyoOAuthConsumer consumer) {
        ...
        resource.addProperty(model.createProperty(CONSUMER_SECRET), consumer.consumerSecret);
