...
require([
  ...
  "dojo/_base/xhr",
  "dojo/ready",
], function (dom, domConstruct, domStyle, on, array, xhr, ready) {
  var pending = {};
  var approved = {};
  ...
  function remove(consumer) {
    if (
      confirm("Delete consumer " + consumer.name + " (" + consumer.key + ")?")
    ) {
      xhr.del({
        url: "consumers/" + encodeURIComponent(consumer.key),
        headers: {
  ...
  function createActions(consumer, node) {
    var actions = consumer.provisional ? PROVISIONAL_ACTIONS : APPROVED_ACTIONS;
  ...
  function createTableRow(consumer, tbody) {
    ...
    var actionsNode = domConstruct.create(
      "td",
      {
