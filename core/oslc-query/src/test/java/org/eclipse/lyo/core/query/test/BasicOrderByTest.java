...
public class BasicOrderByTest {
    static final String prefixes =
            "qm=<http://qm.example.com/ns>," + "olsc=<http://open-services.net/ns/core#>";

    @Test
    public void testOrderBy() throws ParseException {
        String[] expressions = {
            "+gm:priority", "+gm:priority,-oscl:name", "gm:tested_by{+oslc:description}"
        };

        Map<String, String> prefixMap = QueryUtils.parsePrefixes(prefixes);

        for (String expression : expressions) {
            OrderByClause orderByClause = QueryUtils.parseOrderBy(expression, prefixMap);
