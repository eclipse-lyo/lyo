...
public class QueryCapability extends AbstractResource {
    ...
    @OslcDescription(
            "The base URI to use for queries. Queries are invoked via HTTP GET on a query URI"
                    + " formed by appending a key=value pair to the base URI, as described in Query"
    ...
    @OslcDescription(
            "The Query Capability SHOULD provide a Resource Shape that describes the query base"
                    + " URI")
    ...
    @OslcDescription(
            "The expected resource type URI that will be returned with this query capability. These"
    ...
    @OslcDescription(
            "An identifier URI for the domain specified usage of this query capability. If a"
                + " service provides multiple query capabilities, it may designate the primary or"
                + " default one that should be used with a property value of"
