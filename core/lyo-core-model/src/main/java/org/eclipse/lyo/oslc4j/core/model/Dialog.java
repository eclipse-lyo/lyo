...
public class Dialog extends AbstractResource {
    ...
    @OslcDescription(
            "Values MUST be expressed in relative length units as defined in the W3C Cascading"
                + " Style Sheets Specification (CSS 2.1) Em and ex units are interpreted relative"
                + " to the default system font (at 100% size)")
    ...
    @OslcDescription(
            "The expected resource type URI for the resources that will be returned when using this"
                    + " dialog. These would be the URIs found in the result resource's rdf:type"
                    + " property")
    ...
    @OslcDescription(
            "An identifier URI for the domain specified usage of this dialog. If a service provides"
                + " multiple selection or creation dialogs, it may designate the primary or default"
                + " one that should be used with a property value of"
