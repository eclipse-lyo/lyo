...
public class ResourceShapeFactory {
  ...
  private static ResourceShape createResourceShape(
    ...
    final URI about =
        UriBuilder.fromUri(baseURI).path(resourceShapesPath).path(resourceShapePath).build();
        ...
        if (((methodName.startsWith(METHOD_NAME_START_GET))
          ...
          if (propertyDefinitionAnnotation != null) {
            ...
            final Property property =
                createProperty(
                    baseURI, resourceClass, method, propertyDefinitionAnnotation, verifiedClasses);
  ...
  private static Property createProperty(
      ...
      if (genericType instanceof ParameterizedType) {
        ...
        if (actualTypeArguments.length == 1) {
          ...
          if (actualTypeArgument instanceof Class) {
            ...
            log.error(
                "Resource shapes are not designed to be initialized with the help"
                    + " of GenericEntity (or other ways to capture a"
                    + " TypeVariable)");
          }
        }
      }
    ...
    final OslcTitle titleAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcTitle.class);
    if (titleAnnotation != null) {
      property.setTitle(titleAnnotation.value());
    }

    final OslcDescription descriptionAnnotation =
    ...
    final OslcRange rangeAnnotation =
    ...
    final OslcRepresentation representationAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcRepresentation.class);
    ...
    final OslcAllowedValue allowedValueAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcAllowedValue.class);

    if (allowedValueAnnotation != null) {
      property.setAllowedValuesCollection(
          (Collection) Arrays.asList(allowedValueAnnotation.value()));
    }

    final OslcAllowedValues allowedValuesAnnotation =
    ...
    final OslcDefaultValue defaultValueAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcDefaultValue.class);
    if (defaultValueAnnotation != null) {
      property.setDefaultValue(defaultValueAnnotation.value());
    }

    final OslcHidden hiddenAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcHidden.class);
    if (hiddenAnnotation != null) {
      property.setHidden(hiddenAnnotation.value());
    }

    final OslcMemberProperty memberPropertyAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcMemberProperty.class);
    if (memberPropertyAnnotation != null) {
      property.setMemberProperty(memberPropertyAnnotation.value());
    }

    final OslcReadOnly readOnlyAnnotation =
        InheritedMethodAnnotationHelper.getAnnotation(method, OslcReadOnly.class);
    if (readOnlyAnnotation != null) {
      property.setReadOnly(readOnlyAnnotation.value());
    }

    final OslcMaxSize maxSizeAnnotation =
    ...
    final OslcValueShape valueShapeAnnotation =
    ...
    if ((ValueType.LocalResource.equals(valueType))
        || (ValueType.Resource.equals(valueType)
            && (null != representation)
            && (Representation.Inline.equals(representation)))) {
      ...
      if (verifiedClasses.add(componentType)) {
        ...
        createResourceShape(
            baseURI, OslcConstants.PATH_RESOURCE_SHAPES, "unused", componentType, verifiedClasses);
      }
    }
  ...
  private static ValueType getDefaultValueType(
      final Class<?> resourceClass, final Method method, final Class<?> componentType)
  ...
  protected static Class<?> getComponentType(
      final Class<?> resourceClass, final Method method, final Class<?> type)
  ...
  protected static void validateSetMethodExists(
      final Class<?> resourceClass, final Method getMethod)
    ...
    if (!isCollectionType(returnType)) {
      ...
      if (parameterTypes == null || parameterTypes.length == 0 || parameterTypes.length > 1) {
        throw new IllegalArgumentException(
            String.format("Method '%s' shall have exactly one argument", method.getName()));
      }
      ...
      if (!genericReturnType.getActualTypeArguments()[0].equals(
          setterArgumentType.getActualTypeArguments()[0])) {
  ...
  private static void validateUserSpecifiedOccurs(
      final Class<?> resourceClass, final Method method, final OslcOccurs occursAnnotation)
  ...
  protected static void validateUserSpecifiedValueType(
    ...
    // or
    // user-specified value type is non-local resource, and Representation is Inline (we will
    // validate later)
    // or
  ...
  private static void validateUserSpecifiedRepresentation(
    ...
    if (((Representation.Reference.equals(userSpecifiedRepresentation))
      ...
      throw new OslcCoreInvalidRepresentationException(
          resourceClass, method, userSpecifiedRepresentation);
    }
  }
}
