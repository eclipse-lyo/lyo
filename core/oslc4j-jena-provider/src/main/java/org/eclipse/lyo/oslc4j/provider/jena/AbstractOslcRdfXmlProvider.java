...
public abstract class AbstractOslcRdfXmlProvider {
    ...
    protected void writeTo(
        ...
        if ((objects != null && objects[0] != null)
                && (objects[0] instanceof ServiceProviderCatalog
                        || objects[0] instanceof ServiceProvider)
    ...
    private void writeObjectsTo(
        ...
        try {
            ...
            log.trace(
                    "writeObjectsTo - Execution Duration: {} ms",
    ...
    protected void writeTo(
        ...
        final String nextPageURI =
                isClientSide
                        ? null
                        : (String)
                                httpServletRequest.getAttribute(OSLC4JConstants.OSLC4J_NEXT_PAGE);
        final Integer totalCount =
                isClientSide
                        ? null
                        : (Integer)
                                httpServletRequest.getAttribute(OSLC4JConstants.OSLC4J_TOTAL_COUNT);
    ...
    private String getSerializationLanguage(final MediaType baseMediaType) {
        ...
        mediaPairs.add(
                new AbstractMap.SimpleEntry<>(
                        OslcMediaType.TEXT_TURTLE_TYPE, RDFLanguages.strLangTurtle));
        mediaPairs.add(
                new AbstractMap.SimpleEntry<>(
        ...
        mediaPairs.add(
                new AbstractMap.SimpleEntry<>(
                        OslcMediaType.APPLICATION_XML_TYPE, FileUtils.langXMLAbbrev));

        mediaPairs.add(
                new AbstractMap.SimpleEntry<>(
                        OslcMediaType.TEXT_XML_TYPE, RDFLanguages.strLangRDFXML));
    ...
    protected Response buildBadRequestResponse(
        ...
        final MediaType determinedErrorMediaType =
                determineErrorMediaType(initialErrorMediaType, map);
    ...
    private MediaType determineErrorMediaType(
        ...
        try {
            ...
            // Ignore NullPointerException since this means the context has not been set since we
            // are on the client
