 ...
 *
 * <ul>
 * <li>Reification serialization compliant with the <a href="https://docs.oasis-open-projects.org/oslc-op/qm/v2.1/os/quality-management-spec.html#labels">OSLC Quality Management V 2.0 Specification (Labels for Relationships)</a>
 * guidance.</li>
 * <li>Properties:
 * <ul>
...
public class RdfXmlAbbreviatedWriter implements RDFWriterI {
  ...
  public void write(Model model, Writer writer, String base) {
    ...
    // candidates.
    // If a candidate is the object of another root resource candidate, propose it for removal from
    // root resources.
    // If all candidates are proposed for removal (i.e. fully cyclic graph), keep them all try to
    // serialize.
    ...
    // Take removal candidates out of the rootResource list only if all candidates are not proposed
    // for removal.
  ...
  private boolean isChild(
    ...
    if (!isChild && newChildren.size() > 0) {
      ...
      isChild =
          isChild(
              model, visitedResources, parent, (Resource[]) newChildren.toArray(childrenResources));
    }
    return isChild;
  }
  ...
  private void serializeStatement(
    ...
    if ((!RDF.subject.equals(predicate))
      ...
      if (object.isResource()) {
        ...
        // Need to fully retrieve next statement to test if it has been visited to generate the
        // correct XML
        ...
        if (firstResourceStatement != null && !visitedStatements.contains(firstResourceStatement)) {
        ...
        if (RDF.dtXMLLiteral.getURI().equals(literal.getDatatypeURI())) {
          ...
          // FIXME: check if the XML literal type can be detected
  ...
  private static class XMLWriter {
    ...
    public XMLWriter(Writer writer, Model model, int indent, int tab) {
      ...
      // Resolve and add the namespaces defined for the predicate and type object in each statement
      // in the model:
    ...
    public void rootStartTag(String namespaceUri, String localName) {
      ...
      for (Entry<String, String> namespaceEntry : namespaceMap.entrySet()) {
        ...
        final String buffer =
            PREFIX_XMLNS + ':' + namespaceEntry.getValue() + "=\"" + namespaceEntry.getKey() + '"';
