            ...
            for (Resource rootTest: rootTestAgainst){
                ...
                if (!rootTest.equals(rootCandidate)) {
                    ...
                    if (rootCandidateIsObject) {
                        ...
                        boolean objectIsReification = false; // model.listStatements(rootTest, RDF.type, RDF.Statement).hasNext();
...
//            if(!rootResource.canAs(ReifiedStatement.class) && (!serializedResources.contains(rootResource))){
        ...
        while(reifiedStatementsIterator.hasNext()) {
            ...
            if(reifiedStatement.getSubject().isAnon()){
                xmlWriter.attribute(RDF.getURI(), RDF_ATTRIBUTE_ABOUT, ('#' + getShortId(reifiedStatement.getSubject().getId())));
            }
            else{
                xmlWriter.attribute(RDF.getURI(), RDF_ATTRIBUTE_ABOUT, (Util.substituteEntitiesInElementContent(reifiedStatement.getSubject().getURI())));
            }
    ...
    private void serializeStatements(Statement statement, XMLWriter xmlWriter, List<Resource> serializedResources, String rootResourceTypeURI){
        serializeStatements(statement.getResource(), xmlWriter, serializedResources, rootResourceTypeURI);
    }
