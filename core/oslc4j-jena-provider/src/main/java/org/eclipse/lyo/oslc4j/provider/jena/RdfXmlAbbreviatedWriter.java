...
public class RdfXmlAbbreviatedWriter implements RDFWriterI {
     ...
     * @see #ENCODING_UTF8
     */
    private static String XML_DECLARATION_ENCODING =
            "<?xml version=\"1.0\" encoding=\"{0}\"?>"; //$NON-NLS-1$
    ...
    // Namespaces:
    private static String NAMESPACE_URI_RDF =
    ...
    public void write(Model model, Writer writer, String base) {
        ...
        // Note: XML declaration is shown when a) encoding is not UTF8/UTF16 (default) or b)
        // configured.
        if ((showXmlDeclaration == null) || (Boolean.TRUE.equals(showXmlDeclaration))) {
            ...
            if (writer instanceof OutputStreamWriter) {
                ...
                if ((!writerEncoding.equals(ENCODING_UTF8))
                        && (!writerEncoding.equals(ENCODING_UTF16))) {
        ...
        ExtendedIterator<Statement> statementIterator =
                        ...
                        .filterKeep(
                                statement -> {
                                    ...
                                    return ((!statement.isReified())
                                            ...
                                            && (!RDF.subject.equals(predicate))
                                            && (!RDF.predicate.equals(predicate))
        ...
        // Test the candidate root resources to see if they are the objects of other root resource
        // candidates.
        // If a candidate is the object of another root resource candidate, propose it for removal
        // from root resources.
        // If all candidates are proposed for removal (i.e. fully cyclic graph), keep them all try
        // to serialize.
        ...
        for (Resource rootCandidate : rootResources) {
            ...
            for (Resource rootTest : rootTestAgainst) {
                ...
                if (!rootTest.equals(rootCandidate)) {
                    boolean rootCandidateIsObject =
                            model.listStatements(rootTest, null, rootCandidate).hasNext();
                    if (rootCandidateIsObject) {
                        boolean objectIsReification =
                                model.listStatements(rootTest, RDF.type, RDF.Statement).hasNext();
                        if (!objectIsReification) {
                            // checks if there is a self cyclic reference for the resource.
                            boolean isCyclic =
                                    isChild(model, new HashSet<>(), rootCandidate, rootCandidate);
                            if (!isCyclic) {
        ...
        // Take removal candidates out of the rootResource list only if all candidates are not
            ...
            if (!rootResource.canAs(ReifiedStatement.class)
                ...
                if (!rootResource.isAnon()) {
                    xmlWriter.attribute(
                            RDF.getURI(),
                            RDF_ATTRIBUTE_ABOUT,
                            (Util.substituteEntitiesInElementContent(rootResource.getURI())));
                }

                xmlWriter.closeStartTag(true);

                serializeStatements(
                        rootResource, xmlWriter, serializedResources, rootResourceTypeURI);
        ...
        while (reifiedStatementsIterator.hasNext()) {
            ...
            if (reifiedStatement.isAnon()) {
                xmlWriter.attribute(
                        RDF.getURI(),
                        RDF_ATTRIBUTE_ABOUT,
                        ('#' + getShortId(reifiedStatement.getId())));
            } else {
                xmlWriter.attribute(
                        RDF.getURI(),
                        RDF_ATTRIBUTE_ABOUT,
                        (Util.substituteEntitiesInElementContent(reifiedStatement.getURI())));
            }
    ...
    private boolean isChild(
            Model model, Set<Resource> visitedResources, Resource parent, Resource... children) {
        ...
        if (!isChild && newChildren.size() > 0) {
            ...
            isChild =
                    isChild(
                            ...
                            visitedResources,
                            parent,
                            (Resource[]) newChildren.toArray(childrenResources));
        }
        return isChild;
    }
    ...
    private void serializeStatements(
        ...
        StmtIterator statementIterator =
                resource.getModel().listStatements(resource, null, ((RDFNode) (null)));
        ...
        if ((!RDF.subject.equals(predicate))
            ...
            if (object.isResource()) {
                ...
                if ((predicate.isAnon()) && (predicate.getProperty(RDF.type) == null)) {
                    xmlWriter.attribute(
                            RDF.getURI(), RDF_ATTRIBUTE_PARSE_TYPE, RDF_CONSTANT_RESOURCE);
                }
                ...
                // Need to fully retrieve next statement to test if it has been visited to generate
                // the correct XML
                // Convert the statement iterator into a list and use it later to serialize each
                // statement
                ...
                if (firstResourceStatement != null
                        && !visitedStatements.contains(firstResourceStatement)) {
                    ...
                    if (!resource.isAnon()) {
                        xmlWriter.attribute(
                                RDF.getURI(),
                                RDF_ATTRIBUTE_ABOUT,
                                (Util.substituteEntitiesInElementContent(resource.getURI())));
                    }
                ...
                if ((language != null) && (language.length() > 0)) {
                    xmlWriter.attribute(
                            NAMESPACE_URI_XML,
                            RDF_ATTRIBUTE_LANG,
                            (Util.substituteEntitiesInElementContent(language)));
                }
                ...
                if (literal.isWellFormedXML()) {
                    ...
                    xmlWriter.attribute(
                            RDF.getURI(), RDF_ATTRIBUTE_PARSE_TYPE, RDF_CONSTANT_LITERAL);
                    ...
                    if (dataTypeUri != null) {
                        xmlWriter.attribute(
                                RDF.getURI(),
                                RDF_ATTRIBUTE_DATATYPE,
                                (Util.substituteEntitiesInElementContent(dataTypeUri)));
                    }
    ...
    private class XMLWriter {
        ...
        public XMLWriter(Writer writer, Model model, int indent, int tab) {
            ...
            // Resolve and add the namespaces defined for the predicate and type object in each
            ...
            while (statementIterator.hasNext()) {
                ...
                if ((!RDF.subject.equals(predicate))
                    ...
                    if (!namespaceMap.containsKey(namespace)) {
                        namespaceMap.put(
                                namespace, "j." + (unknownNamespacePrefixCounter++)); // $NON-NLS-1$
                    }
                }
            }
        ...
        public void attribute(String namespaceUri, String localName, String value) {
            ...
            final String buffer =
                    " " + namespaceMap.get(namespaceUri) + ':' + localName + "=\"" + value + '"';
            printWriter.print(buffer);
        }
