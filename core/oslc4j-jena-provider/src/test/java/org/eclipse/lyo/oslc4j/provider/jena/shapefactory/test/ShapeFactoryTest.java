...
public class ShapeFactoryTest {
    ...
    private static ShapeWithWorkingReferences createShapeInstanceWithWorkingReferences(String id) {
        ShapeWithWorkingReferences r =
                new ShapeWithWorkingReferences(URI.create("http://test.adaptor.net/r/" + id));
        r.setIdentifier(id);
    ...
    public void invalidShapeWithInlinedResourceTest()
    ...
    public void invalidShapeWithReferencedResourceTest()
            throws OslcCoreApplicationException, URISyntaxException {
        ResourceShape shape =
                ResourceShapeFactory.createResourceShape(
                        "http://localhost:8080",
    ...
    @Test
    public void validInstanceMarshalsAndUnmarshalsAsModelsTest()
        ...
        ShapeWithWorkingReferences resultingResource =
                JenaModelHelper.unmarshalSingle(sourceModel, ShapeWithWorkingReferences.class);
        final Model resultingModel =
                JenaModelHelper.createJenaModel(new Object[] {resultingResource});

        assertThat(sourceModel).isomorphicWith(resultingModel);
    }

    @Test
    public void validInstanceMarshalsAndUnmarshalsAsResourcesTest2()
            throws NoSuchMethodException,
                    IllegalAccessException,
                    IllegalArgumentException,
                    InvocationTargetException,
        ...
        assertTrue(
                sourceResource
                        .getInlines()
                        .getIdentifier()
                        .equals(resultingResource.getInlines().getIdentifier()));
        ...
        assertTrue(
                sourceResource
                        .getInlines()
                        .getAbout()
                        .equals(resultingResource.getInlines().getAbout()));

        // Assert that inlined resources, with multiple cardinality, remains intact.
        InlinedShape[] sourcesInlines =
                sourceResource.getInlinesMany().toArray(new InlinedShape[] {});
        InlinedShape[] resultInlines =
                resultingResource.getInlinesMany().toArray(new InlinedShape[] {});
        for (InlinedShape sourcesInline : sourcesInlines) {
            assertTrue(
                    "Shape "
                            + sourcesInline.getIdentifier()
                            + " missing in the resulting inlined shapes",
        ...
        assertTrue(
                sourceResource
                        .getReferencesAsLocal()
                        .getIdentifier()
                        .equals(resultingResource.getReferencesAsLocal().getIdentifier()));
        ...
        assertTrue(
                sourceResource
                        .getReferencesAsResource()
                        .getValue()
                        .equals(resultingResource.getReferencesAsResource().getValue()));
    }

    private boolean containsResource(InlinedShape[] resultInlines, InlinedShape s1) {
        return Arrays.stream(resultInlines)
                .anyMatch(inlinedShape -> inlinedShape.getIdentifier().equals(s1.getIdentifier()));
    }
}
