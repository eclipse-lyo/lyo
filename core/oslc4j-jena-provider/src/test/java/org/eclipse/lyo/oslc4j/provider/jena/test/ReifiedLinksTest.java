...
public class ReifiedLinksTest {
  ...
  public void testLinkWoLabelIsNotReified() {
    ...
    assertEquals("Only 'linkWithLabel' property should be reified", 1, reifiedStatements.size());
  }
  ...
  private ResourceWithReifiedLinks extract(Model m) {
    try {
      final Object[] objects = JenaModelHelper.fromJenaModel(m, ResourceWithReifiedLinks.class);
      if (objects.length != 1) {
