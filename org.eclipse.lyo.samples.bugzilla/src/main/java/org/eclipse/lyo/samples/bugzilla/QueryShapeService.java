 ...
 */
public class QueryShapeService extends HttpServlet {    	
	private static final long serialVersionUID = -5280734755943517104L; 

    public QueryShapeService() {}
	...
	protected void doGet(
		...
		try {			
			...
			if (AcceptType.willAccept("application/rdf+xml", request)) {
                if ("changerequest".equals(request.getParameter("shape"))) {
                    dispatchTo = "/cm/resourceshape_query_cr_rdfxml.jsp";
                } else {
                    dispatchTo = "/cm/resourceshape_query_rdfxml.jsp";
                }
			} else {
