...
public abstract class TrackedResourceSetService {
    ...
    @Produces(
            {OslcMediaType.TEXT_TURTLE, OslcMediaType.APPLICATION_RDF_XML, OslcMediaType.APPLICATION_XML,
                    OslcMediaType.APPLICATION_JSON})
    public Response getBasePage(@PathParam("page") String pagenum) {
        log.info(
                "received request for base page at url:" + httpServletRequest.getRequestURI() + "" + " . Processing "
                        + "request");
        Base base;
        try {
        ...
        log.info("finished processing request for base page at url:" + httpServletRequest.getRequestURI() + " .");
        log.info("the returned base at url" + httpServletRequest.getRequestURI() + " contains :" + base.getMembers()
                .size() + " members.");
    ...
    public ChangeLog getChangeLogPage(@PathParam("page") String pagenum) {
        log.info(
                "received request for changeLog page at url:" + httpServletRequest.getRequestURI() + " . Processing "
                        + "request");
        ...
        log.info("finished processing request for changeLog page at url:" + httpServletRequest.getRequestURI() + " .");
        log.info(
                "the returned change with url" + httpServletRequest.getRequestURI() + " log " + "contains :" + changeLog
                        .getChange()
                        .size() + " changes.");
        return changeLog;
    }
...
//        URI.create(getServiceBase()
    ...
    public TrackedResourceSet getTrackedResourceSet() throws URISyntaxException {
        log.info(
                "received request for trs at url:" + httpServletRequest.getRequestURI() + " . " + "Processing request");
        ...
        log.info("returning response for request for trs with url:" + httpServletRequest.getRequestURI() + " .");
        log.info(
                "the returned change log at url" + httpServletRequest.getRequestURI() + " " + "contains :" + (
                        (ChangeLog) changeLog)
                        .getChange()
                        .size() + " changes.");
        return result;
    ...
    private String linkHeaderValue(Base base) {
        ...
        String headerValue = urize(pageOf.toString()) + "; rel=\"first\"," + newline + urize(nextPage.getNextPage()
                .toString()) + "; rel=\"next\"," + newline + "<http://www" + "" + "" + "" + ".w3.org/ns/ldp#Page>; "
                + "rel=\"type\"" + newline;
        return headerValue;
    }
