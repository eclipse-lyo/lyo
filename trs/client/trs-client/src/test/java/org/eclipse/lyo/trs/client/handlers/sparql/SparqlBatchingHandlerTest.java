...
public class SparqlBatchingHandlerTest {
  ...
  public void testFinishCycleUsesLyoStore() throws StoreAccessException {
    ...
    try (MockedStatic<StoreFactory> mockedStoreFactory = mockStatic(StoreFactory.class)) {
      ...
      mockedStoreFactory.verify(
          () -> StoreFactory.sparql(null, sparqlUpdateEndpoint, username, password));
  ...
  public void testHandleChangeEventCreation() throws URISyntaxException, StoreAccessException {
    ...
    testModel.add(
        testModel.createResource("http://example.org/resource/1"),
        RDF.type,
  ...
  private BaseMember createTestBaseMember() {
    try {
      ...
      model.add(
          model.createResource(memberUri.toString()),
          RDF.type,
