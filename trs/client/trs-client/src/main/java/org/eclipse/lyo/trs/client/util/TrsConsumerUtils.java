...
public class TrsConsumerUtils {
    ...
    public static List<IProviderHandler> buildHandlersConcurrent(
        ...
        return buildHandlers(
                consumerConfig, providerConfigs, TrsConsumerUtils::concurrentProviderFor);
    }

    public static List<IProviderHandler> buildHandlers(
            final TrsConsumerConfiguration consumerConfig,
            final Collection<TrsProviderConfiguration> providerConfigs,
            final BiFunction<TrsConsumerConfiguration, TrsProviderConfiguration, IProviderHandler>
                    function) {
        ...
        for (TrsProviderConfiguration cfg : providerConfigs) {
            ...
            //                log.warn(
            //                        "Cannot find the internal cutoff event in the Change Logs;
            // treating this as a rebase/rollback scenario.");
            //            } catch (Exception e) {
            //                log.error("Unknown error processing CEs from the TRS Server '{}'",
            // trsProvider);
            //            }
        }
    ...
    private static IProviderHandler providerFor(
        ...
        final IProviderEventHandler handler =
                new SparqlDirectHandler(consumerConfig.getSparqlUpdateUrl());
        IProviderHandler providerHandler =
                new TrsProviderHandler(cfg.getTrsUri(), trsClient, handler);
    ...
    private static IProviderHandler concurrentProviderFor(
        ...
        final ITrackedResourceClient trsClient = trsClientFactory(consumerConfig.getHttpClient());
        final IProviderEventHandler handler =
                new SparqlBatchingHandler(
                        consumerConfig.getSparqlUpdateUrl(),
                        consumerConfig.getSparqlUsername(),
                        consumerConfig.getSparqlPassword());
        final ConcurrentTrsProviderHandler providerHandler =
                new ConcurrentTrsProviderHandler(cfg.getTrsUri(), trsClient, handler);
        return providerHandler;
    }
