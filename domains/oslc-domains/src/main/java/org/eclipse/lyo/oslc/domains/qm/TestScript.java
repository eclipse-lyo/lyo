...
public class TestScript extends AbstractResource implements ITestScript {
    ...
    public String toString(boolean asLocalResource) {
        ...
        if (asLocalResource) {
            result =
                    result
                            + "{a Local TestScript Resource} - update TestScript.toString() to"
    ...
    @OslcDescription(
            "Instructions for executing the test script. Note that the value of Occurs is"
                + " undefined. The resource shape document provided by the QM service provider may"
                + " be consulted for its value.")
    @OslcOccurs(Occurs.ZeroOrMany)
    @OslcValueType(ValueType.Resource)
    @OslcRepresentation(Representation.Reference)
    @OslcReadOnly(false)
