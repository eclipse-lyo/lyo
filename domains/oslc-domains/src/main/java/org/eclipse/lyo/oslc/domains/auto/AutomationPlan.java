...
public class AutomationPlan extends AbstractResource implements IAutomationPlan {
  ...
  public String toString(boolean asLocalResource) {
    ...
    if (asLocalResource) {
      result =
          result
              + "{a Local AutomationPlan Resource} - update AutomationPlan.toString() to present"
              + " resource as desired.";
  ...
  @OslcDescription(
      "The definition of a parameter for this Automation Plan. parameterDefinitions are either a"
          + " local (inline) or referenced resource and use the attributes (the range) of the "
          + " oslc:Property resource with one exception. When used in the context of an"
          + " oslc_auto:parameterDefinition, the cardinality of oslc:propertyDefinition becomes"
          + " zero-or-one instead of exactly-one. Automation consumers creating Automation Requests"
          + " MUST use the oslc:occurs attribute of the parameterDefinition, if present, to"
          + " determine if a given parameter is required when creating the Automation Request. If"
          + " the oslc:occurs attribute indicates the parameter is required (exactly-one or"
          + " one-or-more), the service provider must guarantee the named parameter will be present"
          + " in the Automation Result either as an oslc_auto:inputParmeter when unmodified during"
          + " execution, or as an oslc_auto:outputParameter when modified during execution.")
  @OslcOccurs(Occurs.ZeroOrMany)
  @OslcValueType(ValueType.Resource)
  @OslcReadOnly(false)
  ...
  @OslcDescription(
      "A resource representing the environment(s) which this Automation Plan can be executed in."
          + " The execution environment resource could represent a grouping of environmental"
          + " details such as operating system, database, browser, compiler, etc. See also the"
  ...
  @OslcDescription(
      "A resource representing actions that will become available on Automation Results that result"
          + " from execution of this Plan. The resource is likely to be of type oslc:Action, but it"
          + " can be of any type. Automation defines oslc_auto:TeardownAction as one kind of future"
          + " action.")
  @OslcOccurs(Occurs.ZeroOrMany)
  @OslcValueType(ValueType.Resource)
  @OslcRepresentation(Representation.Reference)
