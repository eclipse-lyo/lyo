...
public class RootServicesHelper {
    ...
    public RootServicesHelper(String url, String catalogDomain, OslcClient client)
            throws RootServicesException {
        this.baseUrl = url;
        this.rootServicesUrl =
                UriBuilder.fromUri(this.baseUrl).path("rootservices").build().toString();
        logger.debug(
                String.format("Fetching rootservices document at URL <%s>", this.rootServicesUrl));
    ...
    private void processRootServices(OslcClient rootServicesClient) throws RootServicesException {
        try {
            Response response =
                    rootServicesClient.getResource(rootServicesUrl, OSLCConstants.CT_RDF);
            ...
            try { // Following field is optional, try to get it, if not found ignore exception
                // because it will use the default
                ...
                logger.debug(
                        String.format(
                                "OAuth authorization realm not found in rootservices <%s>",
                                rootServicesUrl));
            }
            ...
            try {
                this.requestConsumerKeyUrl =
                        getRootServicesProperty(
                                rdfModel,
                                JFS_NAMESPACE,
                                RootServicesConstants.OAUTH_REQUEST_CONSUMER_KEY_URL);
            } catch (ResourceNotFoundException e) {
                logger.debug(
                        String.format(
                                "OAuth request consumer key URL not found in rootservices <%s>",
                                rootServicesUrl));
            }
            ...
            try {
                this.consumerApprovalUrl =
                        getRootServicesProperty(
                                rdfModel,
                                JFS_NAMESPACE,
                                RootServicesConstants.OAUTH_APPROVAL_MODULE_URL);
            } catch (ResourceNotFoundException e) {
                logger.debug(
                        String.format(
                                "OAuth approval module URL not found in rootservices <%s>",
                                rootServicesUrl));
            }
        } catch (Exception e) {
    ...
    public String requestConsumerKey(String consumerName, String consumerSecret)
        ...
        HttpClient client =
                HttpClientBuilder.create()
                        .setRedirectStrategy(
                                new DefaultRedirectStrategy()) // Lax strategy has problems with
                        // HTTPS upgrade
                        .build();
        ...
        if (!response.getFirstHeader("Content-Type").getValue().toLowerCase().contains("json")) {
            ...
            throw new IllegalStateException(
                    "Server returned something else than JSON in the response");
        }
